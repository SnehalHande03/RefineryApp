...
const PredictionDetail = ({ prediction, onBack }) => {
  ...
  const statusIcon = isFailure ? 'AL' : 'OK';
  ...
  return (
    <div className="prediction-detail-container">
      <header className="prediction-header">
        ...
        <div className="timestamp">{new Date(prediction.timestamp).toLocaleString()}</div>
      </header>
        ...
        <section className={`prediction-section status-section ${statusClass}`}>
          ...
          <div className="status-info">
            ...
            <div className="confidence-bar">
              <div className="confidence-fill" style={{ width: `${(prediction.failure_confidence || 0) * 100}%` }}></div>
            </div>
          </div>
        </section>
        ...
        <section className="prediction-section">
          ...
          <div className="sensor-readings-grid">
            <div className="reading-card">
              ...
              {prediction.temperature > 90 && <div className="reading-warning">High</div>}
            </div>
            ...
            <div className="reading-card">
              ...
              {prediction.pressure > 80 && <div className="reading-warning">High</div>}
            </div>

            <div className="reading-card">
              <div className="reading-label">Vibration</div>
              <div className="reading-value">{prediction.vibration.toFixed(2)} mm/s</div>
              <div className="reading-range">Normal: less than 2.3 mm/s</div>
              {prediction.vibration > 2.3 && <div className="reading-warning">Abnormal</div>}
            </div>
                ...
                <div key={idx} className="factor-item">
                  ...
                  <div className="factor-info">
                    ...
                    <div className="factor-bar">
                      <div className="factor-fill" style={{ width: `${factor.importance * 100}%` }}></div>
                    </div>
                  </div>
        ...
        <section className="prediction-section stats-section">
          ...
          <div className="stats-grid">
            <div className="stat-card">
              ...
              <div className={`stat-value ${statusClass}`}>{isFailure ? 'FAILURE RISK' : 'NORMAL'}</div>
            </div>
            <div className="stat-card">
              <div className="stat-label">Sensors Abnormal</div>
              <div className="stat-value">{[prediction.temperature > 90, prediction.pressure > 80, prediction.vibration > 2.3].filter(Boolean).length}</div>
            </div>
            <div className="stat-card">
              <div className="stat-label">Alert Level</div>
              <div className={`stat-value ${isFailure ? 'critical' : 'normal'}`}>{isFailure ? 'CRITICAL' : 'NORMAL'}</div>
            </div>
            <div className="stat-card">
      ...
      <footer className="prediction-footer">
        <button className="back-button large" onClick={onBack}>Return to Dashboard</button>
      </footer>
    </div>
  );
};
